...
const handler = (request, reply) => {
    products.get({}, (err, result) => {
        ...
        products.insert(request.payload, (err, result) => {

            if (err) return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
            /*Salesforce Added By Umesh Beti
            
            
            */
            // delete result.ops.categoryName;
            // delete result.ops.categoryName;
            // delete result.ops.categoryName;
            // delete result.ops.categoryName;
            // delete result.ops.categoryName;
            ...
            // productsElastic.Insert(result.ops[0], (err, resultelastic) => {
            //     if (err) {
            //         logger.error('Error occurred elastci search (Insert): ' + JSON.stringify(err));
            //         return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);//yun
            //     }
            ...
            var DataToSF =
            {
                ...
                "picUrl": result.ops[0].images[0].image,
            ...
            if (authData) {
                superagent
                    .post(authData.instanceUrl + '/services/apexrest/delivx/CentralProduct')
...
const validator = {
    ...
    storeCategoryId: Joi.array().items().required().description('storeCategoryId'),
    ...
    // addOnIds : Joi.any().allow("").description("Add ids if vailable."),
