...
*/
const handler = function (req, reply) { //Action
    ...
    async.waterfall([
        (cb) => {
            jwt.verify(token, config.Secret, (err, decoded) => {
                if (err) {
                    logger.error('Error occurred during driver reset password (verify): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, cb) => {
            verificationCode.isExists({ _id: new ObjectID(decoded.id) }, (err, doc) => {
                if (err) {
                    logger.error('Error occurred during driver reset password (isExists): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, cb) => {
            ...
            driver.isExistsWithId({ _id: new ObjectID(doc.userId) }, (err, user) => {
                if (err) {
                    logger.error('Error occurred during driver reset password (isExistsWithId): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, user, cb) => {
            ...
            driver.setPassword({ _id: new ObjectID(doc.userId), password: password }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during driver reset password (setPassword): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (doc, user, cb) => {
            verificationCode.setLastToken({ _id: new ObjectID(doc._id), lastToken: token }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during driver reset password (setLastToken): ' + JSON.stringify(err));
                    return cb(err);
                }
                return cb(null, user);
            });
        ...
        if (err) {
            logger.error('Error occurred during driver reset password (callback): ' + JSON.stringify(err));
            return reply(err);
        }
        fs.readFile(config.prodEmailTemplateUrl + 'driverpasswordchangebydriver.html', (err, body) => {
            if (err) {
                logger.error('Error occurred during driver reseted password (readFile): ' + JSON.stringify(err));
            }
            var $ = cheerio.load(body);
            let name = result.lastName ? result.firstName + ' ' + result.lastName : result.firstName;
            Object.keys({ username: name, appName: config.appName }).forEach(key => {
                $(key).replaceWith({ username: name, appName: config.appName }[key]);
            });
            if (config.mailGunService == "true") {
                let param = {
                    from: "" + config.appName + " <" + config.emailsFromEmail + ">",
                    email: result.email,
                    subject: config.appName + ' Driver password change',
                    body: $('body').html(),
...
/**
* A module that exports customer reset password handler, validator!
* @exports validator
* @exports handler 
*/
