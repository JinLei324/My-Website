...
const handler = (request, reply) => { //Action
    async.waterfall(
        [
            (callback) => {
                franchise.isExistsCountrycode({ email: request.payload.emailOrMobile, countryCode: request.payload.countryCode, phone: request.payload.emailOrMobile }, (err, doc) => {
                    if (err) {
                        logger.error('Error occurred during franchise forgot password (isExists): ' + JSON.stringify(err));
        ...
        (err, data) => {
            if (err) {
                logger.error('Error occurred during franchise forgot password : ' + JSON.stringify(err));
                return reply(err);
            }
            verificationCode.countForgotCount({ givenInput: data.countryCode + data.phone, userType: 1, triggeredBy: "franchise Forgot Password Verification Code" }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during franchise forgot password (countForgotCount): ' + JSON.stringify(err));
                    // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                    return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                }
                appConfig.get({}, (err, appConfig) => {
                    if (err) {
                        logger.error('Error occurred during franchise send otp(appconfig get): ' + JSON.stringify(err));
                        // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                        return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                    }
                    ...
                    if (result < maxAttemptForgotPassword || result === null) {
                        verificationCode.markStatusFalse({ givenInput: request.payload.countryCode + request.payload.emailOrPhone, userType: 1, triggeredBy: "franchise Forgot Password Verification Code", }, (err, result) => {
                            if (err) {
                                logger.error('Error occurred during franchise forgot password (markStatusFalse): ' + JSON.stringify(err));
                                // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                                return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                            }
                            ...
                            verificationCode.saveVerificationCodeForgotPassword({
                                userType: 1,
                                givenInput: (request.payload.verifyType == 1) ? (request.payload.countryCode + data.phone) : data.email, randomnumber: randomnumber, otpExpiryTime: otpExpiryTime, verifyType: request.payload.verifyType, userId: new ObjectID(data._id), triggeredBy: 'franchise Forgot Password Verification Code', // 2- forgot password ,1 - registration
                            }, (err, result) => {
                                if (err) {
                                    logger.error('Error occurred during franchise forgot password (saveVerificationCodeForgotPassword): ' + JSON.stringify(err));
                                    // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                                    return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                                }
                                switch (request.payload.verifyType) {
                                    case 1:
                                        let params = {
                                            ...
                                            trigger: 'franchise Forgot Password Verification Code (sms)'
                                        }
                                    ...
                                    case 2:
                                        let token = jwt.sign({
                                            id:
                                                ...
                                                result.insertedIds[0].toString(), type: 1
                                        },
                                            config.Secret, { expiresIn: 900 });
                                        if (config.mailGunService == "true") {
                                            email.getTemplateAndSendEmail({
                                                templateName: 'franchiseForgotPassword.html',
                                                toEmail: data.email,
                                                subject: 'Action Required. Reset Password.',
                                                trigger: 'franchise Forgot Password Verification Code',
                                                keysToReplace: { username: data.franchiseName[0].charAt(0).toUpperCase() + data.franchiseName.slice(1), url: "<a href=" + config.forgotpasswordLinkFranchise + "?" + token + " style='color:#12939a' target='_blank'>click here</a>", appName: config.appName }
                                            }, () => {
                                            });
                                        }
...
/**
* A module that exports franchise forgot password handler, validator!
* @exports validator
* @exports handler 
*/
