    ...
    async.waterfall([
        (cb) => {
            jwt.verify(token, config.Secret, (err, decoded) => {
                if (err) {
                    logger.error('Error occurred during franchise reset password (isExists): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, cb) => {
            ...
            //collectionName = (decoded.type === 1) ? 'franchise' : 'franchise';
            franchise.isExistsWithId({ _id: new ObjectID(doc.userId) }, (err, user) => {
                if (err) {
                    logger.error('Error occurred during franchise reset password (isExistsWithId): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, user, cb) => {
            ...
            franchise.setPassword({ _id: new ObjectID(doc.userId), password: password }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during franchise reset password (setPassword): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (doc, user, cb) => {
            verificationCode.setLastToken({ _id: new ObjectID(doc._id), lastToken: token }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during franchise reset password (setLastToken): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        if (err) {
            logger.error('Error occurred during franchise reset password (callback): ' + JSON.stringify(err));
            return reply(err);
        }
        fs.readFile(config.prodEmailTemplateUrl + 'franchisepasswordchangebyfranchise.html', (err, body) => {
            if (err) {
                logger.error('Error occurred during franchise reseted password (readFile): ' + JSON.stringify(err));
            }
            var $ = cheerio.load(body);
            Object.keys({ username: result.franchiseName[0], appName: config.appName }).forEach(key => {
            ...
            if (config.mailGunService == "true") {
                let param = {
                    ...
                    email: result.email,
                    subject: config.appName + ' franchise password change',
                    body: $('body').html(),
...
/**
* A module that exports franchise reset password handler, validator!
* @exports validator
* @exports handler 
*/
