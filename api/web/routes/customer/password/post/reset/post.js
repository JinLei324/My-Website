    ...
    async.waterfall([
        (cb) => {
            jwt.verify(token, config.Secret, (err, decoded) => {
                if (err) {
                    logger.error('Error occurred during customer reset password (isExists): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, cb) => {
            ...
            //collectionName = (decoded.type === 1) ? 'customer' : 'customer';
            customer.isExistsWithId({ _id: new ObjectID(doc.userId) }, (err, user) => {
                if (err) {
                    logger.error('Error occurred during customer reset password (isExistsWithId): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, user, cb) => {
            ...
            customer.setPassword({ _id: new ObjectID(doc.userId), password: password }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during customer reset password (setPassword): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (doc, user, cb) => {
            verificationCode.setLastToken({ _id: new ObjectID(doc._id), lastToken: token }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during customer reset password (setLastToken): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        if (err) {
            logger.error('Error occurred during customer reset password (callback): ' + JSON.stringify(err));
            return reply(err);
        }
        fs.readFile(config.prodEmailTemplateUrl + 'customerpasswordchangebycustomer.html', (err, body) => {
            if (err) {
                logger.error('Error occurred during customer reseted password (readFile): ' + JSON.stringify(err));
            }
            ...
            if (config.mailGunService == "true") {
                let param = {
                    ...
                    email: result.email,
                    subject: config.appName + ' Customer password change',
                    body: $('body').html(),
