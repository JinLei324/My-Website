...
const orderAccepted = require("../../../models/orderAccepted");
...
const handler = (req, reply) => {
    ...
    logger.info("cancel order handler");
    let orderDetails = {};
    let storeData = {};
    let customerData = {};
    var modelName = "";
    ...
    const readNewOrder = newOrder => {
        return new Promise((resolve, reject) => {
                           newOrders.isExistsWithOrderId(
                                                         ...
                                                         logger.info("reading order details from newOrders = "+JSON.stringify(orderDetails)+" for order id "+req.payload.orderId);
    ...
    const readAcceptedOrder = newOrder => {
        return new Promise((resolve, reject) => {
                           orderAccepted.isExistsWithOrderId(
                                                             { orderId: req.payload.orderId },
                                                             (err, res) => {
                                                             orderDetails = res ? res : orderDetails;
                                                             logger.info("reading order details from orderAccepted = "+JSON.stringify(orderDetails));
                                                             modelName = res ? orderAccepted : modelName;
                                                             return err
                                                             ? reject({ message: err, code: 500 })
                                                             : resolve(orderDetails);
                                                             }
    ...
    const readUnassignOrder = newOrder => {
        return new Promise((resolve, reject) => {
                           unassignOrders.isExistsWithOrderId(
                                                              ...
                                                              (err, res) => {
                                                              orderDetails = res ? res : orderDetails;
                                                              logger.info("reading order details from readUnassignOrder = "+JSON.stringify(orderDetails));
    ...
    const readAssignOrder = newOrder => {
        return new Promise((resolve, reject) => {
                           assignOrders.isExistsWithOrderId(
                                                            ...
                                                            (err, res) => {
                                                            orderDetails = res ? res : orderDetails;
                                                            logger.info("reading order details from assignOrders = "+JSON.stringify(orderDetails));
    ...
    const deductCancellationFee = orderDetails => {
        return new Promise((resolve, reject) => {
                           ...
                           logger.info("cancel order admin :"+req.auth.credentials.sub);
                           ...
                           logger.info("cancel order admin position 1");
                           
                           if (req.payload.status == 16) {
                           logger.info("cancel order admin position 16");
                           ...
                           logger.info("cancel order admin position 17");
                           req.payload.cancFee = 0;
                           req.payload.cancCartTotal = parseFloat(0);
                           req.payload.taxes = parseFloat(0);
                           ...
                           logger.info("cancel order admin position 2");
                           req.payload.cancFee = 0;
                           req.payload.cancCartTotal = parseFloat(0);
                           req.payload.taxes = parseFloat(0);
                           req.payload.cancDeliveryFee = parseFloat(0);
                           ...
                           logger.info("cancel order customerData : "+JSON.stringify(customerData));
                           ...
                           logger.info("Outside Total amount charged for cancellation :"+totalAmountToCharge+" and wallet charge : "+walletCharge);
                           ...
                           totalAmountToCharge = totalAmountToCharge - cardCharge;
                           walletCharge += cardCharge;
                           logger.info("Total amount charged for cancellation :"+totalAmountToCharge+" and wallet charge : "+walletCharge);
                           if (totalAmountToCharge > 0 || walletCharge > 0) {
                           walletCharge += totalAmountToCharge;
                           //charge From wallet
                           let walletData = {
    ...
    readNewOrder()
    .then(orderDetails => {
          logger.info("reading order details before = "+JSON.stringify(orderDetails)+" for order length = "+Object.keys(orderDetails).length);
        if (orderDetails === undefined || Object.keys(orderDetails).length == 0) {
          return reply({
    ...
    .then(data => {
          req.payload.statusMsg = 'Cancelled by ' + fromName + ''
    ...
    function sendNotification(data) {
        ...
        notifications.notifyFcmTopic(
                                     ...
                                     notification: "",
                                     //                                     msg: data.reason ? req.i18n.__(req.i18n.__('bookingStatusMsg')[data.status + 'reason'], data.customerDetails.name, data.reason) : req.i18n.__(req.i18n.__('bookingStatusMsg')[data.status + 's'], data.customerDetails.name),
                                     
                                     msg: data.reason ? "Your order for order id "+data.orderId : "Your order for order id "+data.orderId,
        ...
        storeManagers.getAll({ storeId: data.storeId.toString(), status: 2 }, (err, storeManager) => {
                             ...
                             notifications.notifyFcmTopic(
                                                          ...
                                                          notification: "",
                                                          msg: "Your order cancelled for order id "+data.orderId,
                                                          // msg: "Your order has been cancelled",
 ...
 * @exports handler
 */
module.exports = { handler, validator,responseCode };
