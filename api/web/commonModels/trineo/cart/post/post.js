        ...
        if (store) {
            ...
            for (let i = 0; i < productData.units.length; i++) {
                if (String(productData.units[i].unitId) == String(request.payload.unitId)) {
                    ...
                    request.payload.offerName = productData.units[i].offerName;
            ...
            cart.isExists({
                ...
                if (data) {
                    ...
                    cart.isExistsWithItem({
                        ...
                        if (isItem)
                            ...
                            cart.pushItemToCart({
                                ...
                                parentProductId: productData.parentProductId, rx: productData.rx,
                                storeId: storeData._id.toString(), franchiseId: storeData.franchiseId,
                                ...
                                offerName: request.payload.offerName ? request.payload.offerName : 0,
                                taxes: taxes,
                                ...
                                if (err) {
                                    ...
                                    return reply({
                                        ...
                                        data: {
                                            cartId: data ? data.value._id.toString() : "",
                    ...
                    cart.postSave({
                        ...
                        taxes: taxes,
                        offerId: request.payload.offerId ? request.payload.offerId : 0,
                        offerName: request.payload.offerName ? request.payload.offerName : "",
                        customerEmail: customerData.email,
                        customerPhone: customerData.phone,
                        addOns: addOnsData || [],
                        addOnsAdded: addOnFlag || 0,
                        ...
                        if (err) {
                            ...
                            return reply({
                                message: request.i18n.__('cart')['201'],
                                data: {
                                    cartId: res.ops ? data.value._id.toString() : "",
                                    packId: addedToCartOn
                                }
                            }).code(201);
                        }
...
const handlerTest = (request, reply) => {
    let storeData = {};
    let productData = {};
    let customerData = {};
    let zoneData = {};
    ...
    readCustomer(request.auth.credentials.sub == 'customer' ? request.auth.credentials._id : request.payload.customerId).then(readChildProducts).then(readStore).then(readZone).then(store => { // promise to get stores
        ...
        if (store) {
            ...
            cart.isExists({
                ...
                if (data) {
                    switch (data.cartsAllowed) {
                        case 1:
                            if (data.items[0].storeId == storeData._id.toString()) {
                                cart.isExistsWithItem({
                                    userId: request.auth.credentials._id.toString(),
                                    childProductId: request.payload.childProductId,
                                    unitId: request.payload.unitId
                                        ...
                                        cart.pushItemToCart({
                                            ...
                                            if (err) {
                                                ...
                                                return reply({
                                                    message: request.i18n.__('cart')['201'],
                                                    data: {
                                                        cartId: data._id ? data._id.toString() : "",
                                                        packId: addedToCartOn
                                                    }
                                                }).code(201);
                            ...
                            break;
                        case 2:
                            if (storeData.cartsAllowed == 2 && storeData.storeType == data.storeType) {
                                cart.isExistsWithItem({
                                    userId: request.auth.credentials._id.toString(),
                                    childProductId: request.payload.childProductId,
                                    unitId: request.payload.unitId
                        ...
                        case 3:
                            if (storeData.cartsAllowed == 3) {
                                ...
                                return reply({
                                    message: 'different OrderType category'
                                }).code(420);
                            }
                            break;
                        default:
                    ...
                    cart.postSave({
                        ...
                        cartsAllowed: storeData.cartsAllowed,
                        orderTypeMsg: 'Default order',
                        mileageMetric: zoneData.mileageMetric,
                        currencySymbol: storeData.currencySymbol ? storeData.currencySymbol : zoneData.currencySymbol,
                        currency: storeData.currency ? storeData.currency : zoneData.currency,
