            ...
            if (options.key && typeof options.validateFunc === 'function') {
                                ...
                                if (verify_err) {
                                    ...
                                    return options.validateFunc(verify_decoded, request,
                                        function (validate_err, valid, credentials) { // bring your own checks
                                            // logger.warn('not valid func hapi-auth-jwt file ' + valid);
                                            if (validate_err) {
                                            ...
                                            if (valid) {
                                                ...
                                                switch (valid) {
                                                    case 0:
                                                        ...
                                                        break; //cb reject
                                                    case 1: warnMessage = "Your profile got banned by our admin, please contact our support for further queries";
                                                        break; //cb reject
                                                    case 11: warnMessage = "Sorry! You are an guest user.";
                                                        break; //cb reject
                                                    case 2: warnMessage = 'Your profile got rejected by our admin, please contact our support for further queries.';
                                                        break; //cb success
                                                    case 3: warnMessage = 'Your profile is not verified, please contact our support for further queries.';
                                                        break;
                                                    case 5: warnMessage = 'Thanks for signing up. One of our representatives will get in touch with you in the next 24 hours to setup your profile and get all the necessary documents.';
                                                        break;
                                                    case 4: warnMessage = config.sessionMessage;
                                                        break; //cb ban
                                                    case 111: warnMessage = 'Your account is deleted, please contact our support for further queries.';
                                                        break; //cb ban
                                                    case 222: warnMessage = 'You have been logged out by our operations team, please login again to continue accessing your account.';
                                                        break; //cb ban
                                                    default: warnMessage = config.sessionMessage;

                                                };
                                                reply(raiseError('unauthorized',
                                                    warnMessage, tokenType), null,
                                                    { credentials: credentials || verify_decoded }).code(498);
                                            } else {
                                                reply.continue({
                                                    credentials: credentials || verify_decoded,
                                                    artifacts: token
                ...
                return options.verifyFunc(decoded, request,
                    function (verify_error, valid, credentials) {
                        ...
                        if (!valid) {
                            reply(raiseError('unauthorized', config.sessionMessage,
