...
const isExistsDriverIdWithStatus = (params, callback) => {
    db.get().collection(tableName).find({ "orderId": { $in: params.cond ? params.cond : [] } }).sort({ '_id': -1 }).limit(11).skip(0).toArray((err, result) => { // normal select method
        return callback(err, result);
    });

}

const isDriverIdWithStatus = (params, callback) => {
    db.get().collection(tableName).find(params).sort({ '_id': -1 }).limit(11).skip(0).toArray((err, result) => { // normal select method
        return callback(err, result);
    });
...
const isExistsWithCustomerId = (params, callback) => {
    db.get().collection(tableName)
        .findOne({
            ...
            orderId: params.orderId
        }, { orderId: 1, bookingDate: 1, estimatedPackageValue: 1, pickup: 1, drop: 1, status: 1, driverDetails: 1, timeStamp: 1, storeAddr: 1, storeName: 1, statusMsg: 1, customerDetails: 1, totalAmount: 1, Items: 1, drop: 1, paymentType: 1, subTotalAmount: 1, subTotalAmountWithExcTax: 1, exclusiveTaxes: 1, discount: 1, deliveryCharge: 1, storeLogo: 1, storePhone: 1, storeId: 1, storeCoordinates: 1, currency: 1, mileageMetric: 1, currencySymbol: 1, reviewed: 1 }, (err, result) => {
            return callback(err, result);
        });
}
