...
const getProductDetails = (params, callback) => {
    console.log("child product getproductdetails");
...
const getProductsSubCatwise = (params, callback) => {
    ...
    if (params.productName != '' && typeof params.productName != 'undefined') {
        condition['productname.' + params.language] = params.productName;

    }
    if (params.secondCategoryId != '' && typeof params.secondCategoryId != 'undefined') {
        condition.secondCategoryId = params.secondCategoryId;
    }
    if (params.thirdCategoryId != '' && typeof params.thirdCategoryId != 'undefined') {
        condition.thirdCategoryId = params.thirdCategoryId;
    }
...
//         body:
//             {
//                 "query": { "bool": { "must": [{ "match_phrase_prefix": { "storeId": data.storeId } }, { "match_phrase_prefix": { "productName": data.name } }] } },
//                 "_source": ["THC", "CBD", "productName", "price", "storeId", "thumbImage", "mobileImage", "parentProductId", "upc", "sku", "currency", "_id",
//                     "priceValue"]
//             }
//     }, (err, result) => {
...
//             {
//                 $project: {
//                     THC: 1, CBD: 1, productName: 1, price: 1, storeId: 1, thumbImage: 1, mobileImage: 1, parentProductId: 1, upc: 1, sku: 1, currency: 1,
...
const getFavorite = (params, callback) => {
    if (params.storeId == 0) {
        ...
        db.get().collection(tableName)
            ...
            {
                $match: {
                    "stores._id": (params.storeId != 0) ? new ObjectID(params.storeId) : "",
                    "stores.serviceZones": { $in: [params.zoneId] }
                }
            }
                , {
...
const getFavoriteHome = (params, callback) => {
    ...
    if (params.type == 1) { // trending
        ...
        db.get().collection("popularItem")
            .aggregate(
                [
                    { $match: { "storeId": params.storeId != 0 ? new ObjectID(params.storeId) : params.storeId } },
                    { $unwind: "$products" },
                    {
                        $match: {
    ...
    if (params.type == 2) { // highet ofers
        ...
        if (params.storeId != 0) {
            offrCond["storeId"] = params.storeId;
        }
        db.get().collection(tableName)
            .aggregate([{
                $match: offrCond
...
const checkAvailableStock = (params, callback) => {
    let cond = {
        $gt: 0
    };
    if (params.quantity) {
    ...
    db.get().collection(tableName)
        .findOne({
            _id: params._id,
            "units": { $elemMatch: { unitId: params.unitId, availableQuantity: cond } },
            // "units.unitId": params.unitId,
            // "units.availableQuantity": cond
        }, {
            productName: 1,
            productname: 1,
            strainEffects: 1,
...
const readBrands = (params, callback) => {
    db.get().collection(tableName)
        .aggregate([{
            $match: {
                "storeId": (params.storeId != 0) ? new ObjectID(params.storeId.toString()) : "",
                status: { '$in': [1, 6] }
            }
        }, {
            $group: {
...
const getOne = (params, callback) => {
    console.log("child get one params",params);
    db.get().collection(tableName)
