...
const handler = (request, reply) => {
    ...
    stores.getOne({ _id: new ObjectID(request.params.storeId) }, (err, storeData) => {
        ...
        Async.series([
            ...
            function (cb) {
                ...
                if (storeData.driverType == 1) {//freelancer
                    conditionForDriver = { "serviceZones": { "$in": serviceZone }, "driverType": 1, status: 3 };
                } else {//store
                    conditionForDriver = { "storeId": new ObjectID(request.params.storeId), "driverType": 2, status: 3 };
                }
                drivers.readAll(conditionForDriver, { skip: skip, limit: limit }, (err, onlineDrivers) => {
            ...
            function (cb) {
                let conditionForDriver = {};
                if (storeData.driverType == 1) {//freelancer
                    conditionForDriver = { "serviceZones": { "$in": serviceZone }, "driverType": 1, status: 4 };
                } else {//store
                    conditionForDriver = { "storeId": new ObjectID(request.params.storeId), "driverType": 2, status: 4 };
                }
                drivers.readAll(conditionForDriver, { skip: skip, limit: limit }, (err, offlineDrivers) => {
            ...
            function (cb) {
                let conditionForDriver = {};
                if (storeData.driverType == 1) {//freelancer
                    conditionForDriver = { "serviceZones": { "$in": serviceZone }, "driverType": 1, status: 5 };
                } else {//store
                    conditionForDriver = { "storeId": new ObjectID(request.params.storeId), "driverType": 2, status: 5 };
                }
                drivers.readAll(conditionForDriver, { skip: skip, limit: limit }, (err, busyDrivers) => {
                    // cb(null, busyDrivers);
                    var busyPro = [];
            ...
            function (cb) {
                let conditionForDriver = {};
                if (storeData.driverType == 1) {//freelancer
                    conditionForDriver = { "serviceZones": { "$in": serviceZone }, "driverType": 1, status: 8 };
                } else {//store
                    conditionForDriver = { "storeId": new ObjectID(request.params.storeId), "driverType": 2, status: 8 };
                }
                drivers.readAll(conditionForDriver, { skip: skip, limit: limit }, (err, logoutDrivers) => {
            ...
            function (cb) {
                let conditionForDriver = {};
                if (storeData.driverType == 1) {//freelancer
                    conditionForDriver = { "serviceZones": { "$in": serviceZone }, "driverType": 1, status: 9 };
                } else {//store
                    conditionForDriver = { "storeId": new ObjectID(request.params.storeId), "driverType": 2, status: 9 };
                }
            ...
            function (cb) {
                ...
                drivers.readAll(conditionForDriver, { skip: skip, limit: limit }, (err, inactiveDrivers) => {
                    // cb(null, logoutDrivers);
                    var inactivePro = [];
            ...
            webSocket.publish('storeDrivers/' + request.params.storeId, { data: { availableDriver: result[0], offlineDriver: result[1], busyDrivers: result[2], logoutDrivers: result[3], onlineCount: result[4], offlineCount: result[5], busyCount: result[6], logoutCount: result[7], inactiveCount: result[8], inactiveDriver: result[9] } }, { qos: 2 }, (mqttErr, mqttRes) => {
            });

            return reply({ message: request.i18n.__('driverList')['200'], data: { availableDriver: result[0], offlineDriver: result[1], busyDrivers: result[2], logoutDrivers: result[3], onlineCount: result[4], offlineCount: result[5], busyCount: result[6], logoutCount: result[7], inactiveCount: result[8], inactiveDriver: result[9] } }).code(200);
        })
    });
