...
const handler = (request, reply) => { //Action
    async.waterfall(
        [
            (callback) => {
                stores.isExistsCountrycode({ email: request.payload.emailOrMobile, countryCode: request.payload.countryCode, phone: request.payload.emailOrMobile }, (err, doc) => {
                    if (err) {
                        logger.error('Error occurred during storeadmin forgot password (isExists): ' + JSON.stringify(err));
        ...
        (err, data) => {
            ...
            if (err) {
                logger.error('Error occurred during storeadmin forgot password : ' + JSON.stringify(err));
                return reply(err);
            }
            // verificationCode.countForgotCount({ givenInput: data.countryCode + data.ownerPhone, userType: 1, triggeredBy: "storeadmin Forgot Password Verification Code" }, (err, result) => {
            verificationCode.countForgotCount({ givenInput: data.ownerEmail, userType: 1, triggeredBy: "storeadmin Forgot Password Verification Code" }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during storeadmin forgot password (countForgotCount): ' + JSON.stringify(err));
                    // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                    return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                }
                appConfig.get({}, (err, appConfig) => {
                    if (err) {
                        logger.error('Error occurred during storeadmin send otp(appconfig get): ' + JSON.stringify(err));
                        // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                        return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                    }
                    ...
                    if (result < maxAttemptForgotPassword || result === null) {
                        verificationCode.markStatusFalse({ givenInput: request.payload.countryCode + request.payload.emailOrPhone, userType: 1, triggeredBy: "storeadmin Forgot Password Verification Code", }, (err, result) => {
                            if (err) {
                                logger.error('Error occurred during storeadmin forgot password (markStatusFalse): ' + JSON.stringify(err));
                                // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                                return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                            }
                            ...
                            verificationCode.saveVerificationCodeForgotPassword({
                                userType: 1,
                                givenInput: (request.payload.verifyType == 1) ? (request.payload.countryCode + data.ownerPhone) : data.ownerEmail, randomnumber: randomnumber, otpExpiryTime: otpExpiryTime, verifyType: request.payload.verifyType, userId: new ObjectID(data._id), triggeredBy: 'storeadmin Forgot Password Verification Code', // 2- forgot password ,1 - registration
                            }, (err, result) => {
                                if (err) {
                                    logger.error('Error occurred during storeadmin forgot password (saveVerificationCodeForgotPassword): ' + JSON.stringify(err));
                                    // return reply({ message: Joi.any().default(i18n.__('genericErrMsg')['500']) }).code(500);
                                    return reply({ message: request.i18n.__('genericErrMsg')['500'] }).code(500);
                                }
                                switch (request.payload.verifyType) {
                                    case 1:
                                        let params = {
                                            ...
                                            trigger: 'storeadmin Forgot Password Verification Code (sms)'
                                        }
                                    ...
                                    case 2:
                                        ...
                                        if (config.mailGunService == "true") {
                                            ...
                                            email.getTemplateAndSendEmail({
                                                templateName: 'storeadminForgotPassword.html',
                                                toEmail: data.ownerEmail,
                                                subject: 'Action Required. Reset Password.',
                                                trigger: 'storeadmin Forgot Password Verification Code',
                                                keysToReplace: { username: data.ownerName.toUpperCase(), url: "<a href=" + config.forgotpasswordLinkStore + "?" + token + " style='color:#12939a' target='_blank'>click here</a>", appName: config.appName }
                                            }, () => {
                                            });
                                        }
...
/**
* A module that exports storeadmin forgot password handler, validator!
* @exports validator
* @exports handler 
*/
