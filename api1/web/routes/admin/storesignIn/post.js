...
const payload = joi.object({
    ...
    password: joi.string().required().description('store password')
}).required();
...
const handler = (req, reply) => {
    ...
    const getUser = () => {
        return new Promise((resolve, reject) => {
            let condition = { ownerEmail: req.payload.email, password: req.payload.password }
            stores.SelectOne(condition, (err, res) => {
                if (err)
    ...
    const responseData = (data) => {
        return new Promise((resolve, reject) => {
            ...
            let responseArr = {
                token: authToken,
                'storeAdminMqttChannel': configuration.STORE_MQTT_CHANNEL,
            }
            resolve(responseArr);
        });
