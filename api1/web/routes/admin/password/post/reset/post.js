...
const handler = (req, reply) => { //Action
    ...
    async.waterfall([
        ...
        (decoded, doc, cb) => {
            ...
            //collectionName = (decoded.type === 1) ? 'superadmin' : 'superadmin';
            superadmin.isExistsWithId({ _id: new ObjectID(doc.userId) }, (err, user) => {
                if (err) {
                    logger.error('Error occurred during superadmin reset password (isExistsWithId): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (decoded, doc, user, cb) => {
            ...
            superadmin.setPassword({ _id: new ObjectID(doc.userId), password: password }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during superadmin reset password (setPassword): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        (doc, user, cb) => {
            verificationCode.setLastToken({ _id: new ObjectID(doc._id), lastToken: token }, (err, result) => {
                if (err) {
                    logger.error('Error occurred during superadmin reset password (setLastToken): ' + JSON.stringify(err));
                    return cb(err);
                }
        ...
        if (err) {
            logger.error('Error occurred during superadmin reset password (callback): ' + JSON.stringify(err));
            return reply(err);
        }
        fs.readFile(config.prodEmailTemplateUrl + 'superadminpasswordchangebysuperadmin.html', (err, body) => {
            if (err) {
                logger.error('Error occurred during superadmin reseted password (readFile): ' + JSON.stringify(err));
            }
            ...
            if (config.mailGunService == "true") {
                let param = {
                    ...
                    email: result.email,
                    subject: config.appName + ' superadmin password change',
                    body: $('body').html(),
...
/**
* A module that exports superadmin reset password handler, validator!
* @exports validator
* @exports handler 
*/
