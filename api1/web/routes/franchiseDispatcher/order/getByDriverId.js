...
const validator = {
    driverId: Joi.string().required().description('customerId'),
...
const handler = (req, reply) => {
    ...
    let limit = 20;
    let newBooking = {
        "driverId": new ObjectID(req.params.driverId)
    }
    ...
    Async.series([
        function (cb) {
            orders.getAllassign({ q: newBooking, options: { skip: skip, limit: limit } }, 'assignOrders', (err, completedOrdersObj) => {
                cb(null, completedOrdersObj);
            })
        },
        function (cb) {
            orders.count(newBooking, 'assignOrders', (err, newOrderCount) => {
