    ...
    stores.getAllByStoreType(storeParams, (err, result) => {
        ...
        if (result.length > 0) {
            async.each(result, (item, callback) => {
                ...
                console.log("response message : "+util.inspect(storeData, {showHidden: false, depth: null}));
                return reply({
...
if(request.params.serviceType == 1){
    ...
    .then(data => {
        ...
        if (data) {
          console.log("response message : "+util.inspect(data, {showHidden: false, depth: null}));
            return reply({
                message: request.i18n.__('stores')['200'],
                data: data
            }).code(200);
