...
const handlerNew = (request, reply) => {
    ...
    readCustomer(request.auth.credentials.sub == 'customer' ? request.auth.credentials._id : request.payload.customerId).then(readChildProducts).then(readStore).then(readZone).then(store => { // promise to get stores
        ...
        logger.error('err during get fare 111 (catch) ' + JSON.stringify(e));
        if (e.code == 404) {
            return reply({
                message: request.i18n.__('getData')['404']
            }).code(404);
...
const handlerCustomCart = (request, reply) => {
    ...
    if (request.payload.storeType && request.payload.storeType == 7) {

        readCustomer(request.auth.credentials.sub == 'customer' ? request.auth.credentials._id : request.payload.customerId).then(response => {

            console.log("request.auth.credentials.sub - ", request.auth.credentials.sub);
            var storeId = ""
            if (request.auth.credentials.storeId !== 0 || request.auth.credentials.storeId !== '0') {
                storeId = request.auth.credentials.storeId
            }
            let createdBy = '';
            let customerPhone = '';
            let customerEmail = '';
            ...
            let childProductId = String(new ObjectID());
            ...
            cart.isExists({
                ...
                if (data) {
                    ...
                    cart.pushItemToCart({
                        ...
                        quantity: request.payload.quantity,
                        createdBy: request.auth.credentials.sub || 0,
                        appliedDiscount: request.payload.appliedDiscount || 0,
                        offerId: request.payload.offerId ? request.payload.offerId : 0,
                        orderType: request.payload.orderType || "",
                        storeId: storeId,
                        pickUpCoorinates: {
                            latitude: request.payload.pickUpLat || "",
                            longitude: request.payload.pickUpLong || ""
                        },
                        dropCoorinates: {
                            latitude: request.payload.dropLat || "",
                            longitude: request.payload.dropLong || ""
                        },
                        addOnAvailable: 0,
                        storeCoordinates: "",
                        childProductId: childProductId,
                        unitId: childProductId,
                        unitPrice: 0,
                        finalPrice: 0,
                        offerName: "",
                        addOns: [],
                        addOnsAdded: 0,
                        addedToCartOn: childProductId,
                        addOnsAvailableForProduct: 0,
                        taxes: [],
                        storeType: "",
                        cartsAllowed: "",
                        customerEmail: customerData.email || 0,
            ...
            logger.error('err during get fare  222 (catch) ' + JSON.stringify(e));
            if (e.code == 404) {
                return reply({
                    message: request.i18n.__('getData')['404']
                }).code(404);
        ...
        readCustomer(request.auth.credentials.sub == 'customer' ? request.auth.credentials._id : request.payload.customerId).then(readStore).then(readZone).then(store => { // promise to get stores
            ...
            if (store) {
                ...
                request.payload.storeCategoryType = request.payload.storeCategoryType ? request.payload.storeCategoryType : 0;
            ...
            logger.error('err during get fare(catch) 2222', e);
            if (e.code == 404) {
                return reply({
                    message: request.i18n.__('getData')['404']
                }).code(404);
