...
//        $this->load->library('utility_library');
    ...
    function product_details($status = '') {       
       ...
       array(array('$match'=>array('status' => 2)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
    );
            break;
        case 1:    
        ...
        array(array('$match'=>array('status' => 1)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
            ...
            break;
        case 2:    
        $respo = $this->datatables->datatable_mongodbAggregate('BulkImports', array(array('$sort'=>array('_id'=>-1))),
        ...
        if($status==0 || $status==1 ){
            ...
            foreach ($aaData as $value) {
                ...
                $arr[] =  $secondCategoryName;
                $arr[] = $thirdCategoryName;
                $arr[] = '<button class="btn btn-primary btnWidth editProducts cls111" style="width:35px;"  value=' . $value['_id']['$oid'] . '><i class="fa fa-edit"></i></button>';
                $arr[] =$createdOn;
                $arr[] =$success;
                $arr[] = $failed;
                $arr[] =$repeated;
    ...
    function datatableStoreDetails($status) {
        ...
        switch ($status) {
            case 0:
                $respo = $this->datatables->datatable_mongodbAggregate('childProducts', array(array('$match'=>array('status' => 0)),array('$sort'=>array('_id'=>-1))),
                array(array('$match'=>array('status' => 0)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
            );
                break;

            case 1:
                $respo = $this->datatables->datatable_mongodbAggregate('childProducts', array(array('$match'=>array('status' => 1)),array('$sort'=>array('_id'=>-1))),
                array(array('$match'=>array('status' => 1)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
            );
                break;

            case 2:
                $respo = $this->datatables->datatable_mongodbAggregate('childProducts', array(array('$match'=>array('status' => 2)),array('$sort'=>array('_id'=>-1))),
                array(array('$match'=>array('status' => 2)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
            );
                break;

            case 3:
                $respo = $this->datatables->datatable_mongodbAggregate('childProducts', array(array('$match'=>array('status' => 3)),array('$sort'=>array('_id'=>-1))),
                array(array('$match'=>array('status' => 3)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
            );
                break;

            case 4:
                $respo = $this->datatables->datatable_mongodbAggregate('childProducts', array(array('$match'=>array('status' => 4)),array('$sort'=>array('_id'=>-1))),
                array(array('$match'=>array('status' => 4)),array('$group'=>array('_id'=>1,count=>array('$sum'=>1))))
        ...
        foreach ($aaData as $value) {
            ...
            $setComEn = $value['commissionEnable']=='1' ? 'checked': '' ;            
            $setComButton =  $value['commissionEnable']=='1' ?'display:block;':'display:none;' ;

            $arr[] = '<div class="switch cls111" data-id="' . $value['_id']['$oid'] . '" style="margin-top: 14px;float: left;"><input id="' . $value['_id']['$oid'] . 'x"  class="cmn-toggle cmn-toggle-round" type="checkbox" style="display: none;" ' . $setComEn . '><label for="' . $value['_id']['$oid'] . 'x"></label></div><div class="pull-right m-t-10 new_button cls111" style="display:block" > <button class="commissionSettings setbtn btn-info"  style="'.$setComButton.'" slaveid = "' . $value['_id']['$oid'] .'">Set</button></div>';            

            $setComValue = $value['commissionEnable']=='1' ? $value['commissionValue']: 'Disabled' ;
            $arr[]='<div class="comvalue" id="'.$value['_id']['$oid'] .'">'.$setComValue.'</div>';
...
//               print_r($result); die;
    ...
    function AddNewProductData() {
        ...
        $data['storeCategoryId'] = explode(',', $data['storeCategoryId']);
...
//        print_r($data);die;
    ...
    function getBulkinfo($bulkId){
     ...
     function getboxtype(){
        $data = $this->mongo_db->where(array('status'=>1))->get('packageBox');
        $record =[];
        foreach($data as $rec){
            $record[] = $rec; 
        }
    ...
    public function getCommission() {
        ...
        $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($pass_id)))->find_one('childProducts');
        
        $data=array();
        if($result){
            
            if (isset($result['commissionValue'])) 
                $commissionValue = $result['commissionValue'];
            

            $data = array('commissionValue' => $commissionValue, 'returnType' => true);
        }
    ...
    public function commissionUpdate() {
        $this->load->library('mongo_db');
        $pass_id = $this->input->post('pass_id');
        $pass_value = $this->input->post('pass_value');
        $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($pass_id)))->find_one('childProducts');

        $r = false;
        if ($result) {
            $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($pass_id)))->set(array('commissionValue' => $pass_value))->update('childProducts');
        ...
        echo json_encode(array('comValue'=>$pass_value, 'returnType' => $r));
        return;
    }

    public function commissionEnable(){
        $this->load->library('mongo_db');
        $pass_id = $this->input->post('pass_id');
        $commissionEnable = $this->input->post('pass_enable');
        $result = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($pass_id)))->find_one('childProducts');
        ...
        if ($result) {
            ...
            if(isset($result['commissionValue'])){
                $comValue = $result['commissionValue'];
            }else{
                $comValue = 40;
            }
            
            $data = $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($pass_id)))->set(array('commissionEnable' => $commissionEnable, 'commissionValue'=>$comValue))->update('childProducts');
        ...
        echo json_encode(array('comValue' => $comValue, 'returnType' => $r));
        return;
    }
