...
class FranchiseLevelCategorymodal extends CI_Model {
...
//        $count1 = count($cat);
        ...
        $respo = $this->datatables->datatable_mongodb('franchisefirstCategory', array("addedFrom"=>"franchise",'status' => (int) $status), '_id', -1); //1->ASCE -1->DESC
        ...
        foreach ($aaData as $value) {   
            ...
            $franchiseName=$this->mongo_db->where(array('_id'=>new MongoDB\BSON\ObjectID($franchiseID)))->select(array("name"=>"name"))->find_one('franchise'); 
           // print_r($franchiseName['name']['en']);die;
...
//            echo '<pre>';    print_r($data); die;
	...
	function approveCategory(){
		 ...
		 foreach($val as $id){
            $dataFirstCat = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->find_one('franchisefirstCategory');
            unset($dataFirstCat['_id']);
            unset($dataFirstCat['addedFrom']);
            $franchiseCat = $this->mongo_db->insert('firstCategory', $dataFirstCat);
            $res = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->set(array('status'=>1))->update('franchisefirstCategory');
		 }
		 if($res){
			 echo json_encode(array('msg'=>1));
		 }else{
	...
	function banCategory(){
		 ...
		 foreach($val as $id){
			$res = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->set(array('status'=>4,'reason'=>$reason))->update('firstCategory');
		 }
		 if($res){
			 echo json_encode(array('msg'=>1));
		 }else{
	...
	function rejectCategory(){
		 ...
		 $reason = $this->input->post('reason');
		 foreach($val as $id){
			$res = $this->mongo_db->where(array("_id" => new MongoDB\BSON\ObjectID($id)))->set(array('status'=>3,'reason'=>$reason))->update('franchisefirstCategory');
		 }
		 if($res){
			 echo json_encode(array('msg'=>1));
		 }else{
...
//        echo '<pre>';       print_r($data);   die;
    ...
    function deleteCategory() {
        ...
        $image = $data['imageUrl'];
        foreach ($val as $row) {
            $this->mongo_db->where(array('_id' => new MongoDB\BSON\ObjectID($row)))->set(array("status"=>2))->update('firstCategory');
            $this->mongo_db->where(array('categoryId' => $row))->set(array("status"=>2))->update('firstCategory');
            $this->mongo_db->where(array('categoryId' => $row))->set(array("status"=>2))->update('firstCategory');
        }
