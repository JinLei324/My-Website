...
foreach ($language as $val)
    if (array_search($val['code'], $elan) != false)
...
//                </div>');
    ...
    $(document).ready(function () {
        ...
        $.each(lan, function (ind, val) {
            var html = '<div class="form-group form-group-default required cat_lan_' + val.code + '">\
                            <label><?= $ttl_cat ?>Category (' + val.name + ')</label>\
                            <input type="text" required name="cat_name[' + val.code + ']" id="cat_name_' + val.code + '" value="' + data.name[val.code] + '" class="form-control" >\
                        </div>';
            $(html).insertAfter($('.maincat'));
        });
        $('#cat_name').val(data.name['en']);
        if (data.has_scat == false) {
            $('#desc_name').val(data.desc['en']);
            $.each(lan, function (ind, val) {

            var html = '<div class="form-group form-group-default required cat_lan_' + val.code + '">\
                            <label>Description (' + val.name + ')</label>\
                                                <textarea required name="desc[' + val.code +']" id="desc_name' + val.code +'" class="desc_name1">'+data.desc[val.code]+'</textarea></div>';
                                 
                               
            $(html).insertAfter($('.maindesc'));
             CKEDITOR.replace('desc['+ val.code+']');
        });

        }
        else
        {
            $('#cat_subcat').prop('checked', true);
        }
        ...
        $('.lan_check').change(function () {
            ...
            if ($(this).is(':checked')) {
                ...
                var html = '<div class="form-group form-group-default required cat_lan_' + $(this).val() + '">\
                            ...
                            <input type="text" name="cat_name[' + $(this).val() + ']" id="cat_name_'+ $(this).val() +'"  class="form-control">\
                        </div>';
                $(html).insertAfter($('.maincat'));
...
<div class="content">
    <div class="" data-pages="parallax">
        <div class="container-fluid container-fixed-lg sm-p-l-20 sm-p-r-20">
            <div class="inner" style="transform: translateY(0px); opacity: 1;">
               <h5 class="" style="color:#2a3f54;font-weight:900"><?= strtoupper($this->lang->line('pagetitlehelp'));?></h5>
            </div>
        </div>
    </div>
    <div class="container-fluid container-fixed-lg">
        <ul class="breadcrumb" style="padding: 10px 5px;background: white;">
            <li>
                <a class="active" href="<?= base_url() ?>index.php?/helpText"><?= ucwords($this->lang->line('helpText'));?></a>
            </li>
            <li>
                <a href="#" class="active">
        ...
        <div class="container-md-height m-b-20">
            <div class="panel panel-default" style="border:0;">
                <form action="<?= base_url() ?>index.php?/helpText/support_actionCustomer" method='post' novalidate>
                                    ...
                                    foreach ($language as $val) {
                                        ...
                                        <div class='col-sm-3'>
                                            <div class="checkbox check-success <?= $landis ?>">
                                                <input <?= $s ?> type="checkbox" class='lan_check' value="<?= $val['code'] ?>" id="checkbox<?= $val['code'] ?>" data-id='<?= $val['name'] ?>' <?= $landis ?>>
                                                <label for="checkbox<?= $val['code'] ?>"><?= $val['name'] ?></label>
                                            </div>
                                        </div>
                                    <?php } ?>
                                </div>
