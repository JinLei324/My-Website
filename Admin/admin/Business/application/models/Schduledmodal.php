...
class Schduledmodal extends CI_Model {
    ...
    public function datatable_getProviderDetails($status = '',$timeOffset = '') {
        ...
        $timeOffset = $timeOffset * 60; // now it convert  in minutes
           ...
           $dateTimestamp = strtotime($date);
            $workingHourStoreDoc = $this->mongo_db->where(array('status'=> 1,'storeId'=>$this->session->userdata('badmin')['BizId'], 'endDateTimestamp'=>['$lt'=>$dateTimestamp]))->get('workingHour') ;
        ...
        switch((int)$status){
            case 1:
            $respo = $this->datatables->datatable_mongodb('workingHour',array('storeId'=>$this->session->userdata('badmin')['BizId'],'status'=> 1),'','');
            break;
            case 2:
            $respo = $this->datatables->datatable_mongodb('workingHour', array('storeId'=>$this->session->userdata('badmin')['BizId'],'status' => 2),'','');
            break;
            case 3:
            $respo = $this->datatables->datatable_mongodb('workingHour', array('storeId'=>$this->session->userdata('badmin')['BizId'],'status' => 3),'','');
            break;
    ...
    //was commented because error in datatable    
    ...
    function addSchduled() {
       ...
       // $data['duration'] = $this->input->post('duration');
        ...
        //  calling API to store the data in Particular stores
            ...
            $hour['storeId'] =$this->session->userdata('badmin')['BizId'];
