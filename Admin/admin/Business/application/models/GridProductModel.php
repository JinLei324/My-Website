...
class GridProductModel extends CI_Model {
    ...
    function get_product($storeId){
    
       $this->load->library('mongo_db');
      $res=$this->mongo_db->where(array("storeId" => new MongoDB\BSON\ObjectId($storeId),"status"=>1))->get('childProducts');
    ...
    public function getCategory(){
       ...
       $data=$this->mongo_db->where(array('visibility'=>1,'storeid'=>$storeId))->get('storefirstCategory');
    ...
    public function getCatlist(){
            ...
            $data=$this->mongo_db->where(array('visibility'=>1,'storeid'=>$storeId,"status" => 1,))->get('storefirstCategory');
            ...
            foreach( $data as $Categorys){ 
                ...
                $entities .= '<option value="' . $Categorys['_id']['$oid'] . '" data-name="'.$Categorys['name']['0'].'" catName="'.$Categorys['name']['0'].'" >' . $Categorys['name']['0'] . '</option>';
    ...
    public function getSubCategory(){
            ...
            foreach( $data as $subCategorys){ 
                ...
                $entities .= '<option value="' . $subCategorys['_id']['$oid'] . '" data-name="'.$subCategorys['name']['0'].'" catName="'.$subCategorys['name']['0'].'" >' . $subCategorys['name']['0'] . '</option>';
    ...
    public function getCategorylist($categoryId,$storeId){
        ...
        $data=$this->mongo_db->where(array('firstCategoryId'=> $categoryId,'storeId'=>new MongoDB\BSON\ObjectID($storeId),'status'=>1))->get('childProducts');
    ...
    public function getSubCategorylist($categoryId,$storeId){
        ...
        $data=$this->mongo_db->where(array('secondCategoryId'=> $categoryId,'storeId'=>new MongoDB\BSON\ObjectID($storeId),'status'=>1))->get('childProducts');
    ...
    public function getSubSubCategorylist($categoryId,$storeId){
        ...
        $data=$this->mongo_db->where(array('thirdCategoryId'=> $categoryId,'storeId'=>new MongoDB\BSON\ObjectID($storeId),'status'=>1))->get('childProducts');
        $currencySymbol = $this->mongo_db->where(array("_id"=>new MongoDB\BSON\ObjectID($storeId)))->select(array("currencySymbol"=>"currencySymbol"))->find_one('stores');
        // echo '<pre>';
        // print_r( $data);die;
        echo json_encode(array('data'=>$data,'currencySymbol'=>$currencySymbol));
    ...
    public function getManufacturerslist($manufracturerId,$storeId){
       ...
       // chnages done in grid product


      //get_Grid product view on click of grid button
      function productGridView($status,$storeId){
    
        $res=$this->mongo_db->where(array("status" =>(int) $status,"storeId" =>new MongoDB\BSON\ObjectId($storeId)))->limit(10)->offset(0)->get('childProducts');
        $currencySymbol = $this->mongo_db->where(array("_id"=>new MongoDB\BSON\ObjectID($storeId)))->select(array("currencySymbol"=>"currencySymbol"))->find_one('stores');
     ...
     function get_productPagination($offset=""){        
        $limit=10;
        $res=$this->mongo_db->where(array("status" => 1))->limit($limit)->offset($offset*10)->get('childProducts');       
        echo json_encode(array('data'=>$res));
       }
